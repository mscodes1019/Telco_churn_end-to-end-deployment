...
def validate_telco_data(df) -> Tuple[bool, List[str]]:
    """
    Data validation for Telco Customer Churn dataset using Great Expectations (v0.18.12).
    """

    print("🔍 Starting data validation with Great Expectations...")

    # Clean numeric columns (fix type issues before validation)
    ...
    if results["success"]:
        print(f"✅ Data validation PASSED: {len(results['results'])} checks successful")
    else:
